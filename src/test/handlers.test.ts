...
const checkMainAccountExists = async (
...
describe("AISA SDK Multi-Account Workflow", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      "Created agent B with public key:",
      agentBKeypair.publicKey.toString()
    );
    console.log(
      "Created agent C with public key:",
      agentCKeypair.publicKey.toString()
    );
    ...
    while (!mainAccountCreated && retries < MAX_RETRIES) {
      try {
        ...
        const accountExists = await checkMainAccountExists(
          mainAccountHandler,
          MULTI_TEST_UUID
        );
        ...
        if (accountExists) {
          console.log(
            "Multi-test main account already exists with correct owner, continuing..."
          );
          ...
          console.log(
            "Attempting to create multi-test main account with UUID:",
            MULTI_TEST_UUID.join(",")
          );
          try {
            const txId = await mainAccountHandler.createMainAccount(
              MULTI_TEST_UUID,
              [TEST_PAYEE], // Add main wallet and test payee to whitelist
              [] // no restrictions for token whitelist
            );
            ...
            const verifyExists = await checkMainAccountExists(
              mainAccountHandler,
              MULTI_TEST_UUID
            );
            if (verifyExists) {
              console.log(
                "Successfully verified multi-test main account creation"
              );
              mainAccountCreated = true;
              break; // Exit the loop immediately
            } else {
              ...
              console.log(
                "Account verification failed, generating new UUID and retrying..."
              );
            ...
            if (
              createError.toString().includes("DuplicatePubkey") ||
              (createError.transactionLogs &&
                createError.transactionLogs.some((log: string) =>
                  log.includes("DuplicatePubkey")
                ))
            ) {
              console.log(
                "Account with this UUID exists but has different owner, generating new UUID..."
              );
              MULTI_TEST_UUID.length = 0; // Clear the array
              MULTI_TEST_UUID.push(...generateTestUUID()); // Add new values
            } else {
              ...
              console.log(
                "Unexpected error creating account, generating new UUID and retrying..."
              );
              MULTI_TEST_UUID.length = 0; // Clear the array
              MULTI_TEST_UUID.push(...generateTestUUID()); // Add new values
            }
        ...
        console.log(
          "Error in account check, generating new UUID and retrying..."
        );
        MULTI_TEST_UUID.length = 0; // Clear the array
        MULTI_TEST_UUID.push(...generateTestUUID()); // Add new values
        retries++;
    ...
    if (mainAccountCreated) {
      ...
      console.warn(
        "Multi-account test setup failed, tests in this suite will be skipped"
      );
    }
  ...
  test("Should create sub-accounts for agents B and C", async () => {
    ...
    if (!mainAccountCreated) {
      console.log(
        "Main account not created, skipping sub-account creation test"
      );
      return;
    }
    ...
    try {
      ...
      const subAccountB = deriveSubAccount(
        mainAccount,
      ...
      try {
        ...
        console.log(
          "Sub-account for agent B already exists, skipping creation"
        );
        ...
        const txIdB = await mainAccountHandler.createSubAccount(
          ...
          }, // paymentInterval (1 day in seconds)
        ...
        try {
          await mainAccountHandler.program.account.subAccount.fetch(
            subAccountB
          );
      ...
      console.log(
        "Continuing despite sub-account B creation failure, but later tests may fail"
      );
    }
    ...
    try {
      // First check if the sub-account already exists
      const mainAccount = deriveMainAccount(Uint8Array.from(MULTI_TEST_UUID));
      const subAccountC = deriveSubAccount(
        mainAccount,
      ...
      try {
        ...
        console.log(
          "Sub-account for agent C already exists, skipping creation"
        );
        ...
        const txIdC = await mainAccountHandler.createSubAccount(
          ...
          }, // paymentInterval (12 hours in seconds)
      ...
      // Continue with tests but log clear error
      console.log(
        "Continuing despite sub-account C creation failure, but later tests may fail"
      );
    }
  }, 20000);
  ...
  test("Should set token allowances for agents B and C", async () => {
    ...
    if (!subAccountBCreated && !subAccountCCreated) {
      console.log(
        "No sub-accounts were created, skipping allowance setting test"
      );
      return;
    }
    ...
    if (subAccountBCreated) {
      try {
        ...
        if (error.toString().includes("DuplicatePubkey")) {
          console.log(
            "This appears to be a duplicate key error, which may be safe to ignore"
          );
        } else {
          console.error("Failed to set allowance for agent B:", error);
          console.log(
            "Continuing despite agent B allowance setting failure, but later tests may fail"
          );
        }
      }
    } else {
    ...
    if (subAccountCCreated) {
      try {
        ...
        if (error.toString().includes("DuplicatePubkey")) {
          ...
          console.log(
            "Continuing despite agent C allowance setting failure, but later tests may fail"
          );
        }
      }
    } else {
  ...
  test("Should check and fund sub-accounts with tokens", async () => {
    ...
    try {
      ...
      if (subAccountBCreated) {
        ...
        console.log(
          "Sub-account B token account:",
          subAccountBTokenAccount.toString()
        );
      }
      ...
      if (subAccountCCreated) {
        ...
        console.log(
          "Sub-account C token account:",
          subAccountCTokenAccount.toString()
        );
      }
      ...
      console.log(
        "Main wallet token account:",
        mainWalletTokenAccount.toString()
      );
      ...
      try {
        ...
        const mainBalance =
          await mainAccountHandler.connection.getTokenAccountBalance(
            mainWalletTokenAccount
          );
        ...
        console.log(
          "Using the tokens transferred by increaseAllowance - skipping additional transfers"
        );
      } catch (error: any) {
      ...
      console.error(
        "Failed to check and fund sub-accounts with tokens:",
        error
      );
      console.log(
        "Continuing despite token funding issues, but payment tests may fail"
      );
    }
  }, 10000);
  ...
  test("Should initialize handlers for agents B and C", async () => {
    ...
    if (!mainAccountCreated) {
      console.log(
        "Main account not created, skipping handler initialization test"
      );
      return;
    }
    ...
    class CustomBHandler extends SubAccountTxHandler {
      public static async initializeWithKeypair(
        ...
        const handler = await SubAccountTxHandler.initializeWithKeypair(
          keypair
        );
        return handler;
      }
    ...
    if (subAccountBCreated) {
      try {
        ...
        agentBHandler = await CustomBHandler.initializeWithKeypair(
          agentBKeypair
        );
        console.log("Initialized handler for agent B");
        expect(agentBHandler).toBeDefined();
        expect(agentBHandler.wallet.publicKey.toString()).toBe(
          agentBKeypair.publicKey.toString()
        );
      ...
      console.log(
        "Sub-account B not created, skipping its handler initialization"
      );
    }
    ...
    if (subAccountCCreated) {
      try {
        // Initialize the handler with the custom keypair
        agentCHandler = await CustomCHandler.initializeWithKeypair(
          agentCKeypair
        );
        console.log("Initialized handler for agent C");
        expect(agentCHandler).toBeDefined();
        expect(agentCHandler.wallet.publicKey.toString()).toBe(
          agentCKeypair.publicKey.toString()
        );
      ...
      console.log(
        "Sub-account C not created, skipping its handler initialization"
      );
    }
  ...
  test("Should process payment requests from agents B and C", async () => {
    ...
    if (!agentBHandlerInitialized && !agentCHandlerInitialized) {
      console.log(
        "No agent handlers were initialized, skipping payment request test"
      );
      return;
    }
    ...
    if (agentBHandlerInitialized) {
      try {
        ...
        try {
          const stateB = await agentBHandler.getSubAccountState(
            MULTI_TEST_UUID
          );
          console.log("Agent B sub-account state before payment:", stateB);
        } catch (error: any) {
          console.warn(
            "Could not verify agent B sub-account state, payment may fail:",
            error
          );
        }
        ...
        console.log(
          "Agent B payment request failed, check previous errors for context"
        );
      }
    } else {
      console.log(
        "Agent B handler not initialized, skipping its payment request"
      );
    }
    ...
    if (agentCHandlerInitialized) {
      try {
        // First verify the sub-account exists and has appropriate allowance/state
        try {
          const stateC = await agentCHandler.getSubAccountState(
            MULTI_TEST_UUID
          );
          console.log("Agent C sub-account state before payment:", stateC);
        } catch (error: any) {
          console.warn(
            "Could not verify agent C sub-account state, payment may fail:",
            error
          );
        }
        ...
        console.log(
          "Agent C payment request failed, check previous errors for context"
        );
      }
    } else {
      console.log(
        "Agent C handler not initialized, skipping its payment request"
      );
    }
  }, 30000);
  ...
  test("Should verify sub-account states after payments", async () => {
    ...
    if (!agentBHandlerInitialized && !agentCHandlerInitialized) {
      console.log(
        "No agent handlers were initialized, skipping sub-account state verification test"
      );
      return;
    }
    ...
    if (agentBHandlerInitialized) {
      try {
        ...
        console.log(
          "Agent B last payment timestamp:",
          lastPaymentTimestampB.toString()
        );
      } catch (error: any) {
        console.error("Failed to verify agent B sub-account state:", error);
        console.log(
          "Sub-account state verification failure is expected if earlier steps failed"
        );
      }
    } else {
      console.log(
        "Agent B handler not initialized, skipping its state verification"
      );
    }
    ...
    if (agentCHandlerInitialized) {
      try {
        ...
        console.log(
          "Agent C last payment timestamp:",
          lastPaymentTimestampC.toString()
        );
      } catch (error: any) {
      ...
      console.log(
        "Agent C handler not initialized, skipping its state verification"
      );
    }
  });
});
